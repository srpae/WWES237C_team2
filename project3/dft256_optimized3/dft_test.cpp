...
#ifdef IN_PLACE
#define OUTPUT_R In_R
#define OUTPUT_I In_I
#else
#define OUTPUT_R Out_R
#define OUTPUT_I Out_I
#endif
...
int main()
{
	...
	for(int i=0; i<SIZE; i++)
	{
		...
		rmse_R.add_value((float)OUTPUT_R[i] - gold_R);
		rmse_I.add_value((float)OUTPUT_I[i] - gold_I);
	}
	fclose(fp);
